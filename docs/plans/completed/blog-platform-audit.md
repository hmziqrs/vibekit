...
| Category and tag management                           | **PARTIAL**     | Tag system fully implemented (CRUD API, tag selector in editor, tag filtering on blog index, tag display on posts). **No category system** — tags serve as the sole taxonomy. Tag update endpoint added (`PATCH /api/blog/tags/:id`) with slug regeneration and conflict detection.                                                                                                                                                                                   |
...
9. ~~**Blog post contentBody stored raw/unsanitized**~~ **FIXED** — Both create and update endpoints now call `sanitizeHtml()` on `contentBody` before storing. Strips dangerous tags (`script`, `iframe`, `style`, `form`, `input`, `textarea`, `select`, `button`) and attributes (`style`, `formaction`, `xlink:href`, event handlers). Safe HTML (paragraphs, links, code blocks, images) is preserved.

10. ~~**Scheduled publish cron doesn't re-index posts to FTS**~~ **FIXED** — The `POST /api/admin/publish-scheduled` endpoint now calls `indexBlogPost()` for each published post alongside cache purging and audit logging. Previously only direct publish (`/:id/publish`) triggered FTS indexing.

11. ~~**No tag update endpoint**~~ **FIXED** — Added `PATCH /api/blog/tags/:id` with `updateTagSchema` validator. Regenerates slug from name, checks for slug conflicts against other tags, writes audit log. Previously tags could only be created or deleted.
