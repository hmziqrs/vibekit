...
| Tax configuration          | **PARTIAL**         | Local tax calculation (`calculateTax` with inclusive/exclusive modes). Schema fields `taxInclusive`, `taxRate`. Stripe automatic_tax plumbing exists but not enabled at checkout.                                     |
