...
| 2   | Team member management (invite, roles, permissions) | Partial     | Member invite works (email + role selection). Role change and member removal work. RBAC permission checks via `hasPermission()` and `requirePermission()` middleware. **Invitation acceptance UI added** at `/app/invitations` — lists pending invitations with Accept button, calls `POST /invitations/:token/accept`. "Invitations" link added to app sidebar nav. **Still missing**: Invitation management UI for senders (view pending, cancel, re-send). |
...
2. ~~**No invitation acceptance UI**~~ **DONE** — `/app/invitations` page added with list of pending invitations and Accept button. "Invitations" link in app sidebar nav.
