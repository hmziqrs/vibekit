...
| Session management               | **DONE**     | Sessions work (create, list, revoke, refresh). IP/UA columns exist but may not be populated on creation. Max 5 concurrent sessions enforced in `hooks.server.ts`, oldest evicted on new login.                           |
