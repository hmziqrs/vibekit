...
* [ ] All secrets from §8.8 catalogued in `wrangler.jsonc` / `wrangler secret put`
* [ ] Better Auth configured with Drizzle adapter + `additionalFields`
* [ ] `bun auth:schema` produces `src/lib/server/db/auth.schema.ts`
* [ ] Paraglide JS configured
* [ ] Route groups `(public)`, `(blog)`, `(auth)`, `(app)`, `(admin)` scaffolded
* [ ] `hooks.server.ts` session + guard scaffold + security headers
* [ ] Cloudflare Rate Limiting rules configured for auth + contact endpoints
* [ ] Shared Zod validators module (`src/lib/validators/**`)
* [ ] `cn` + `cva` helpers
...
## 19.4 Blog
* [ ] Blog schema + migration (including `deleted_at`, `blog_post_slug_history`)
* [ ] Tag / category schema (if included)
* [ ] Markdown rendering pipeline (`micromark` + GFM + DOMPurify)
* [ ] R2 + Cloudflare Images upload integration
* [ ] `/blog` index route
* [ ] `/blog/[slug]` detail route + slug-history 301 fallback
* [ ] Slug uniqueness validation
* [ ] Draft save
* [ ] Publish
* [ ] Update
* [ ] Archive (`status = 'archived'`)
* [ ] Soft-delete (sets `deleted_at`) + restore
