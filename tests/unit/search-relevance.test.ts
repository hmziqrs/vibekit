...
describe('search relevance weights', () => {
  it('has default weights with title boosted highest', () => {
    expect(DEFAULT_WEIGHTS.title).toBe(10)
    expect(DEFAULT_WEIGHTS.content).toBe(1)
    expect(DEFAULT_WEIGHTS.metadata).toBe(0.5)
    expect(DEFAULT_WEIGHTS.entityType).toBe(0)
  })
  ...
  it('allows custom weight overrides', () => {
    const custom: SearchWeights = { ...DEFAULT_WEIGHTS, title: 20, content: 2 }
    expect(custom.title).toBe(20)
    expect(custom.content).toBe(2)
