...
This enables eager loading: \`db.query.blogPost.findMany({ with: { author: true } })\``,
    ...
    seoDescription: 'Master type-safe database queries with Drizzle ORM and Cloudflare D1 SQLite.',
...
function generateSql(): string[] {
  ...
  for (const post of POSTS) {
    ...
    const publishedAt =
      post.status === 'published' ? "cast(unixepoch('subsecond') * 1000 as integer)" : 'NULL'
...
async function runWrangler(filePath: string) {
  ...
  const exitCode = await new Promise<number>((resolve) => {
    const proc = spawn(
      'npx',
      ['wrangler', 'd1', 'execute', 'vibekit-db', '--local', '--file', filePath],
      {
        shell: false,
        stdio: 'inherit',
      }
...
async function cleanSeed() {
  ...
  for (const slug of POSTS.map((p) => p.slug)) {
    lines.push(
      `DELETE FROM blog_post_tag WHERE post_id IN (SELECT id FROM blog_post WHERE slug = '${esc(slug)}');`
    )
