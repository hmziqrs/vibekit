...
const PAGES = [
  { path: '/', expected: 'prerendered-no-csr', group: '(public)' },
  { path: '/features', expected: 'prerendered-no-csr', group: '(public)' },
  { path: '/pricing', expected: 'prerendered-no-csr', group: '(public)' },
  { path: '/about', expected: 'prerendered-no-csr', group: '(public)' },
  { path: '/privacy', expected: 'prerendered-no-csr', group: '(public)' },
  { path: '/terms', expected: 'prerendered-no-csr', group: '(public)' },
...
function findPre(dir: string, base = ''): Array<{ path: string; size: number; html: string }> {
  const r: Array<{ path: string; size: number; html: string }> = []
  if (!existsSync(dir)) return r
  for (const e of readdirSync(dir)) {
    const fp = join(dir, e)
    const s = statSync(fp)
    const rp = join(base, e)
    if (s.isDirectory()) {
      r.push(...findPre(fp, rp))
    } else if (e === 'index.html' || e.endsWith('.html')) {
      const p = rp.replace(/index\.html$/, '').replace(/\.html$/, '')
      r.push({ path: p === '' ? '/' : '/' + p, size: s.size, html: readFileSync(fp, 'utf-8') })
    }
  }
  return r
}
...
async function main() {
  ...
  console.log(`  Prerendered .html files found: ${preFiles.length}`)
  console.log('══════════════════════════════════════════════════════════════\n')
  ...
  for (const [group, pages] of groups) {
    ...
    for (const page of pages) {
      const pre = preMap.get(page.path)
      const notes: string[] = []
      let ok = false

      if (page.expected === 'prerendered-no-csr') {
        if (!pre) { notes.push('❌ Expected prerendered but NO .html found'); fail++ }
        else {
          ok = true
          notes.push(`✅ Prerendered HTML ${pre.size} bytes`)
          if (hasHydration(pre.html)) notes.push('⚠️  Hydration scripts found in prerendered HTML')
          else notes.push('✅ No hydration scripts (csr=false confirmed)')
          pass++
        }
      } else if (page.expected === 'csr-only') {
        if (pre) { notes.push('❌ CSR-only page unexpectedly has prerendered .html'); fail++ }
        else { ok = true; notes.push('✅ Not prerendered — shell rendered at request time'); pass++ }
      } else if (page.expected === 'ssr-with-csr') {
        if (pre) { notes.push('❌ SSR page unexpectedly has prerendered .html'); fail++ }
